...
| `MOTHERDUCK_TOKEN` | MotherDuck warehouse access token |
| `MOTHERDUCK_DATABASE` | MotherDuck database (default `student_label_analytics`) |
| `MOTHERDUCK_HOST` | Postgres endpoint host (default `pg.us-east-1-aws.motherduck.com`) |
...
Use `/api/health/deep` for monitoring and paging (Admin session or Bearer `HEALTH_PROBE_SECRET` / `CRON_SECRET`) — it returns **503** when MongoDB or core env is unhealthy (avoid using it as the ALB check if optional integrations cause flapping). Keep `/api/health` as the unauthenticated ALB target.
