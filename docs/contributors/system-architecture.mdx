...
| Auth | NextAuth.js v4 — credentials, JWT sessions, optional TOTP MFA |
...
| --- | --- | --- |
| Authentication | `/auth/signin`, `/api/auth/*` | Login, MFA, sessions |
| Intake | `/intake`, `/api/intake/*` | Student registration at front desk |
...
| `users` | Accounts (bcrypt passwords, MFA) |
...
| ThoughtSpot | App → cloud + embed | Optional analytics liveboard |
...
| `GET /api/health/deep` | **Readiness** — MongoDB, env vars, integration config |
...
<CardGroup cols={2}>
  ...
  <Card title="API health checks" icon="heart-pulse" href="https://student-label-system.vercel.app/api/health/deep">
...
- [Cursor and MCP setup](/contributors/cursor-and-mcp) — contributor tooling
