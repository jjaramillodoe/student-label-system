...
description: Configure school1.yourdomain.org portals — DNS, Vercel, env vars, and School Settings slugs.
---
...
Until you set `TENANT_ROOT_DOMAIN`, the app keeps working on the current host only (for example [student-label-system.vercel.app](https://student-label-system.vercel.app/)). School isolation still comes from **user school assignment** and School Settings — subdomains are the portal URL layer.
...
1. Open `https://yourdomain.org/auth/signin` — district sign-in (no school badge required)
2. Open `https://school1.yourdomain.org/auth/signin` — should show **School 1** (or the name for that slug)
