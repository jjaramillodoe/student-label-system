...
<Tip>
  On **Duplicates**, **Merge** offers a short **Undo** window (~10 seconds) if the wrong record was removed.
</Tip>
...
<Warning>
  **MFA is required for password sign-in** until DOE Single Sign-On (SSO) is available. All staff (Admin, Data Lead, Data Member, Intake Member) must enable authenticator-app MFA. Accounts without MFA are limited to Profile until they enroll.
</Warning>
...
- Admins review failed logins and MFA events at **Admin → Security** (`/admin/security`); repeated failures can email notification recipients
- After **8** failed password/MFA attempts, an account is locked for about **30 minutes** (or unlock from Users → Security / Admin → Security)
- Creating a user (or bulk upload) logs a security event and can email notification recipients
