...
1. Admin sets the slug under **School Settings** (defaults like `school1`…`school8` for the built-in schools).
2. DNS: point `*.yourdomain.org` at Vercel and add the wildcard domain once.
3. Env: `TENANT_ROOT_DOMAIN`, `NEXT_PUBLIC_TENANT_ROOT_DOMAIN`, and `NEXTAUTH_COOKIE_DOMAIN=.yourdomain.org`.

The apex host (and the current Vercel URL) still work for district-wide Admin use. School staff can bookmark their portal URL. Wrong-school logins show a warning banner.
