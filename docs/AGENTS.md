...
- Auth security: credentials login forces MFA enrollment (`forceMfaSetup` → Profile) unless Admin sets per-user `mfaBypass`; JWT `maxAge` 12h; `auth_events` + Admin → Security; lockout after 8 failures (~30m); email alerts on repeated failures / lockout / MFA disable / new user
...
- Avery labels show a **5-digit print sequence**, then **Last, First**, DOB, Label ID barcode, and QR — packed to the top of the sticker for hanging-file tabs
