...
async function buildLabelCell(s: StudentData | null, sequence?: number): Promise<TableCell> {
  ...
  const innerRows = seqText
    ? (() => {
        ...
        const minTop = LABEL_DOCX_TEXT_MARGIN_TOP + NAME_LINE + LABEL_DOCX_NAME_AFTER + DOB_LINE + LABEL_DOCX_DOB_AFTER;
        const topRowH = Math.max(
          minTop,
          Math.round(INNER_H / 2 - LABEL_SEQ_NUDGE_TOP_TWIPS - seqRowH / 2),
        );
        const botRowH = Math.max(200, INNER_H - topRowH - seqRowH);
        return [
          new TableRow({
    ...
    if (!parsed.skipStock) {
      try {
        await recordPrintHistoryAndConsume(db, {
          ...
          layout: 'avery5163',
          status: 'completed',
          note: 'Avery 5163 Word download',
        });
      } catch (stockErr) {
        console.error('[avery5163-docx] stock consume failed', stockErr);
      }
    }
